<template>
  <div class="full-width p20 bg-white hz-low-code">
...
export default {
  ...
  computed: {
      body(){
        debugger
        let searchFields=deepCopy(searchFieldsTmpl)
        searchFields.push({
              key:'tip',
              type:"FormText",
              wraperProperties:{
         ...
         return [
          {
            ...
            name:'statisticType',
            props: appendToPreset('searchOption',
              {
                class:['grid-col-24',],
                options:{
                   properties: {
  ...
  methods: {
    ...
    loadData(){
      protectLevStatisticApi().then((data)=>{
        this.pageData.protectLev= {'等级':data.map((item)=>{
      ...
      growthSituationStatisticApi().then((data)=>{
        this.pageData.growStatus= {'使用状况':data.map((item)=>{
          return{value:item.countNumber,label:item.categoryName}
        })}
      ...
      treeAgeStatisticApi().then((data)=>{
          ...
          this.pageData.treeAge={'使用年数':data.map((item)=>{
              return {value:item.countNumber,label:item.categoryName}
            })}
      ...
      treeSpeciesStatisticApi().then((data)=>{
        this.pageData.treeSpecies={'资产类型':data.map((item)=>{

<template>
    <main class="flex relative search-container">
        <DynamicFormContent ref="searchOption" :form-item-list="formFields" v-bind="options"
                            :form-properties="options.properties" :data="data" :col-num="4" @formDataUpdated="change"
        >
            <template #actionBtnSlot>
                <section class="  ">
                    <el-button v-if="options.trigger=='click'" class="lh10" type="primary" size="small" icon="el-icon-search" @click="submit">{{ options.searchLabel||'搜索' }}</el-button>
                    <el-button v-if="options.resetable" class="lh10" size="small" icon="el-icon-refresh" @click="reset">重置</el-button>
                </section>
            </template>
        </DynamicFormContent>
        <el-button v-if="options.mainNum<fields[0].children.length" class="ml12 align-self-start " type="text" size="small" @click="switchSearchBar">
            高级筛选
